...
export default function Macro() {
  ...
  const timeout = async () => await new Promise((resolve) => setTimeout(resolve, 20)); 
  ...
  useEffect(() => {
      if (swapDivRef.current[0] !== -1 && swapDivRef.current[1] !== -1) {
          setElementDelta(
              (document.getElementById("position" + swapDivRef.current[1])?.getBoundingClientRect().x ?? 0) -
              (document.getElementById("position" + swapDivRef.current[0])?.getBoundingClientRect().x ?? 0) 
          );
